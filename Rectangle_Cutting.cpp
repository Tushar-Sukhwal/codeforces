...
int recc(int a, int b, vector<vector<int>> &dp) {
  if (a == b) return 0;
  ...
  for (int i = 1; i < a; i++) {
    ans = min(ans, recc(i, b, dp) + recc(a - i, b, dp));
  }

  for (int i = 1; i < b; i++) {
    ans = min(ans, recc(a, i, dp) + recc(a, b - i, dp));
  }
  ...
  for (int i = 1; i < n + 1; i++) {
    for (int j = 1; j < m + 1; j++) {
      ...
      for (int ti = 1; ti < i; ti++) {
        ans = min(ans, dp[ti][j] + dp[i - ti][j]);
      }
      for (int tj = 1; tj < j; tj++) {
        ans = min(ans, dp[i][tj] + dp[i][j - tj]);
      }
      dp[i][j] = ans + 1;
    }
  }
