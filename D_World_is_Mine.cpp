...
int recc(int a, int b, vector<int> &arr, vector<vector<int>> &dp) {
  if (a == arr.size()) return 0;

  if (dp[a][b] != -1) return dp[a][b];

  // bob eats
  if (b >= arr[a]) {
    return dp[a][b] = max(1 + recc(a + 1, b - arr[a], arr, dp),
                          recc(a + 1, b + 1, arr, dp));
  } else {
    return dp[a][b] = recc(a + 1, b + 1, arr, dp);
  }
}
...
int32_t main() {
  ...
  test {
    ...
    cout << arr.size() - recc(0, 0, arr, dp) << endl;
    // _print(dp);
  }
  return 0;
}
