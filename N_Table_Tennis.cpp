...
jaiShreeRam {
  letsgooooooooooo lli n, k;
  cin >> n >> k;
  deque<int> arr(n);
  fi cin >> arr[i];

  lli prevWinner, currWinner, opponent, count = 0;

  prevWinner = max(arr[0], arr[1]);
  opponent = min(arr[0], arr[1]);
  currWinner = prevWinner;
  count++;
  arr.pop_front();
  arr.pop_front();

  arr.push_back(opponent);
