...
jaiShreeRam {
  letsgooooooooooo test {
    ...
    lli totalDamage = k;
    lli totalReduction = 0;
    lli currIndex = 0;
    lli i = 0;

    // fi { cout << v[i].first << " " << v[i].second << endl; }

    while (1) {
      while (i < n) {
        if (v[i].first > totalDamage) {
          // cout << v[i].first << " " << v[i].second << endl; 
          totalReduction += v[i].second;
          totalDamage +=( k - totalReduction);
          // cout << totalDamage << endl;
          break;
        }
        i++;
      }
      if (totalDamage <= 0 && i != n) {
        no break;
      }
